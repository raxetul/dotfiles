...
#
# Bridges the gap between the static GPG config in
# configurations/gpg/ (which knows how to use a key) and the real,
# per-host key material that has to live outside the repo. Running this
# flips signing from off to on; not running it leaves things functional
# but unsigned.
#
...
#      `include`d by the repo's gitconfig (symlinked into ~/.config/git/
#      by scripts/symlinks.sh), so signing turns on with zero further
#      config.
...
command -v gpg >/dev/null 2>&1 || fatal "gpg not on PATH. Run ./setup.sh first to install GPG."
...
# We rewrite the file in full every run so the wizard is the single
# source of truth for signing config. The repo's gitconfig (symlinked
# into ~/.config/git/ by scripts/symlinks.sh) includes this path, so
# editing it never fights the repo.
cat > "${INCLUDE_PATH}" <<EOF
# Generated by scripts/gpg-setup.sh. Edits will be overwritten on the
# next run of the wizard. Included from configurations/git/gitconfig.
[user]
    signingkey = ${KEY_ID}
[commit]
    gpgsign = true
